...
export default class PluginLoader {
  ...
  async traverseDirectory(dir) {
    try {
      ...
      logger.error(chalk.red('[群聊洞见]') + ' 扫描目录失败:', chalk.yellow(dir))
  ...
  async loadAll() {
    ...
    try {
      ...
      if (jsFiles.length === 0) {
        logger.warn(chalk.yellow('[群聊洞见] 未找到任何插件文件'))
        return this.getResult()
      }

      logger.info(chalk.blue('[群聊洞见]') + ` 找到 ${chalk.cyan(jsFiles.length)} 个插件文件，开始加载...`)
      ...
      logger.error(chalk.red('[群聊洞见] 加载过程出错:'), err)
    }
