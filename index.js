...
export class GroupManager extends plugin {
   ...
   * 测试表情格式（诊断工具）
   */
  async testEmojiFormat(e) {
    await e.reply('✅ 表情格式测试模式已激活！\n\n请发送包含以下内容的消息（可以分多条）：\n1. QQ 系统表情（例如：😊 小表情）\n2. 动画表情（大表情/sticker）\n3. Unicode Emoji（例如：😀🎉）\n\n我会在日志中详细记录消息结构。')

    // 设置一个临时的监听器来捕获下一条消息
    const testHandler = async (testEvent) => {
      // 只处理同一个群的消息
      if (testEvent.group_id !== e.group_id) return

      try {
        logger.mark('========== 表情格式测试开始 ==========')
        logger.mark(`[测试] 消息来源: 群 ${testEvent.group_id}, 用户 ${testEvent.user_id}`)
        logger.mark(`[测试] 消息文本: ${testEvent.msg}`)
        logger.mark(`[测试] 原始消息 (raw_message): ${testEvent.raw_message}`)

        // 输出事件对象的关键字段
        if (testEvent.raw) {
          logger.mark(`[测试] raw 字段类型: ${typeof testEvent.raw}`)
          if (typeof testEvent.raw === 'string') {
            logger.mark(`[测试] raw 内容: ${testEvent.raw}`)
          } else {
            logger.mark(`[测试] raw 对象: ${JSON.stringify(testEvent.raw, null, 2)}`)
          }
        }

        logger.mark(`[测试] 消息段数量: ${testEvent.message.length}`)
        logger.mark('---------- 消息段详细信息 ----------')

        // 遍历每个消息段
        testEvent.message.forEach((msg, index) => {
          logger.mark(`\n[消息段 ${index + 1}]`)
          logger.mark(`  类型 (type): ${msg.type}`)

          // 输出所有字段
          logger.mark(`  所有字段: ${Object.keys(msg).join(', ')}`)

          // 根据类型输出关键信息
          if (msg.type === 'text') {
            logger.mark(`  文本内容: ${msg.text}`)
          } else if (msg.type === 'face') {
            logger.mark(`  face.id: ${msg.id}`)
            logger.mark(`  face.data: ${JSON.stringify(msg.data || 'undefined')}`)
            if (msg.raw) {
              logger.mark(`  face.raw 类型: ${typeof msg.raw}`)
              logger.mark(`  face.raw 内容: ${typeof msg.raw === 'string' ? msg.raw : JSON.stringify(msg.raw)}`)
            }
          } else if (msg.type === 'image' || msg.type === 'img') {
            logger.mark(`  image.url: ${msg.url || 'undefined'}`)
            logger.mark(`  image.file: ${msg.file || 'undefined'}`)
            logger.mark(`  image.summary: ${msg.summary || 'undefined'}`)
            logger.mark(`  image.sub_type: ${msg.sub_type || 'undefined'}`)
          } else if (msg.type === 'mface' || msg.type === 'marketface') {
            logger.mark(`  ${msg.type}.url: ${msg.url || 'undefined'}`)
            logger.mark(`  ${msg.type}.file: ${msg.file || 'undefined'}`)
            if (msg.data) {
              logger.mark(`  ${msg.type}.data: ${JSON.stringify(msg.data)}`)
            }
          }
          ...
          if (msgJson.length > 500) {
            logger.mark(`  完整 JSON (前500字符): ${msgJson.substring(0, 500)}...`)
          } else {
            logger.mark(`  完整 JSON: ${msgJson}`)
          }
        })

        logger.mark('========== 表情格式测试结束 ==========\n')

        // 移除监听器（只处理一次）
        Bot.off('message.group', testHandler)

      } catch (err) {
        logger.error(`[测试] 测试过程出错: ${err}`)
    ...
    // 注册临时监听器
    Bot.on('message.group', testHandler)

    // 30秒后自动移除监听器（防止泄漏）
    setTimeout(() => {
      try {
        Bot.off('message.group', testHandler)
        logger.debug('[测试] 表情格式测试监听器已自动清理')
      } catch (err) {
        // 忽略错误
      }
    }, 30000)
   ...
   * 查看表情统计（测试命令）
   */
  async viewEmojiStats(e) {
    if (!e.isGroup) {
      await e.reply('此功能仅支持群聊使用', true)
      return false
    }

    if (!messageCollector || !statisticsService) {
      await e.reply('统计功能未就绪', true)
      return false
    }
    ...
    try {
      ...
      // 构建统计报告
      let report = `📊 当天表情统计报告\n\n`
      report += `总消息数: ${stats.basic.totalMessages}\n`
      report += `总表情数: ${stats.basic.totalEmojis}\n\n`
      report += `--- 表情分类 ---\n`
      report += `QQ 表情 (face): ${stats.emoji.face}\n`
      report += `动画表情 (mface): ${stats.emoji.mface}\n`
      report += `Unicode Emoji: ${stats.emoji.emoji}\n`
      report += `总计: ${stats.emoji.total}\n\n`

      // 检查数据一致性
      if (stats.emoji.total !== stats.basic.totalEmojis) {
        report += `⚠️ 警告：统计不一致！\n`
        report += `emoji.total (${stats.emoji.total}) != basic.totalEmojis (${stats.basic.totalEmojis})\n\n`
      }
      ...
      if (stats.topUsers.length > 0) {
        ...
        stats.topUsers.slice(0, 5).forEach((user, index) => {
          if (user.emojiCount > 0) {
            report += `${index + 1}. ${user.nickname}: ${user.emojiCount} 个表情\n`
          }
        })
      }
      ...
      logger.error(`[群聊助手] 查看表情统计失败: ${err}`)
      await e.reply(`查看统计失败: ${err.message}`, true)
      return false
    }
  }
}
